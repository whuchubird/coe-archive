...
if (!connectionString) {
  throw new Error('DATABASE_URL이 없다. .env.example을 복사해 .env를 만들고 값을 채운다.');
}
...
// 풀이 들고 있던 유휴 연결이 끊겼을 때 프로세스가 죽지 않도록 잡아둔다.
pool.on('error', (err) => {
  console.error('[db] 유휴 연결 오류:', err.message);
});

// 서버 기동 시 DB에 실제로 닿는지 확인한다. 실패하면 예외를 던져 호출부가 판단한다.
