...
추가 기능 명세 (v3) — 공개 테이스팅 노트

기존 CLAUDE.md 하단에 이어붙인다. 앞선 원칙·스키마·디자인 방향은 그대로 유효하다.

개념

사용자가 작성한 테이스팅 노트를 다른 사용자도 볼 수 있게 한다. 지금까지 노트는 작성자 본인만 볼 수 있었으므로, 이 기능으로 서비스가 개인 기록장에서 커뮤니티 성격을 갖게 된다.

노트는 작성 시 공개 / 비공개를 선택한다.
...
비공개 노트도 목록에는 나타나되 세부 내용이 가려진다. 평점 통계에는 공개·비공개 모두 포함된다 — 평점 자체는 어느 쪽이든 공개 항목이기 때문이다.

스키마 변경
sql
ALTER TABLE notes ADD COLUMN is_public BOOLEAN NOT NULL DEFAULT TRUE;
CREATE INDEX idx_notes_bean_created ON notes(bean_id, created_at DESC);

기본값을 TRUE로 두어 기존 노트는 공개로 전환된다. 이미 작성된 노트를 사용자 동의 없이 공개로 바꾸는 셈이므로, schema.sql에도 컬럼을 반영하되 기존 데이터의 처리 방침을 화면에 안내한다. 발표·제출용 프로젝트라 실제 사용자가 없다면 문제되지 않지만, 정책적 판단이 필요한 지점이라는 사실을 보고서에 기록한다.

인덱스는 로트별 최신순 조회가 이 기능의 주 질의 패턴이므로 추가한다.

API
GET /api/beans/:id/notes?page=1&limit=10

인증 불필요. 비로그인 방문자도 노트를 볼 수 있다.
...
json
{
  "items": [
    {
      ...
      "isMine": false,
      "brewMethod": "V60",
      "comment": "리치와 장미가 선명하다",
      "sensory": { "aroma": 4.5, "acidity": 5.0, ... }
    },
    {
      "id": 41,
  ...
  "page": 1,
  "totalPages": 4,
  "summary": { "avgRating": 4.3, "count": 37 }
}
서버에서 반드시 지킬 것

비공개 노트의 세부 필드는 응답에서 아예 제외한다. null로 채워 보내거나, 전체를 내려보내고 프론트에서 가리는 방식은 금지한다. 개발자 도구로 응답을 열면 그대로 보이기 때문이다. 이 구분은 SQL의 CASE 또는 조회 후 매핑 단계에서 수행한다.

sql
CASE WHEN n.is_public THEN n.comment END AS comment

본인 노트는 예외다. isMine: true인 경우 비공개여도 세부 내용을 내려준다. 자기가 쓴 글은 볼 수 있어야 한다. 이 판단은 세션의 userId와 notes.user_id를 비교해 서버에서 결정한다.

정렬은 최신순 고정. created_at DESC. 페이지네이션은 기존 /api/beans와 동일한 방식(LIMIT/OFFSET)을 쓴다.

summary.avgRating은 전체 노트 기준으로 계산한다. 현재 페이지의 10건이 아니라 해당 로트의 모든 노트다. 별도 집계 쿼리로 산출한다.

노트 작성·수정 API 변경
POST /api/notes    → 요청 본문에 isPublic (기본 true)
PUT  /api/notes/:id → isPublic 수정 가능

isPublic은 불리언으로 검증한다. 문자열 "false"가 참으로 평가되지 않도록 주의한다.

화면 — detail.html

위치 — Similar profiles 섹션 바로 아래, 페이지 최하단.
...
작성자 아이디, 별점, 작성일, 추출 방식 배지, 코멘트, 감각 6축을 가로 막대나 소형 수치로 표시. 6축을 매 카드마다 레이더로 그리면 화면이 무거워지므로 레이더는 쓰지 않는다.

노트 카드 (비공개)

작성자 아이디, 별점, 작성일, 그리고 "비공개 노트입니다" 안내. 잠금 아이콘이나 흐림 처리로 시각적으로 구분하되, 세부 영역을 흐리게 처리하는 방식은 쓰지 않는다 — 가릴 내용 자체가 없어야 한다.

본인 노트

isMine이면 "내 노트" 배지를 붙이고, 비공개여도 내용을 보여준다. 수정 버튼으로 notes.html로 이동시킨다.

빈 상태

노트가 0건이면 "아직 작성된 노트가 없습니다"와 함께, 로그인 상태면 작성 유도 버튼을, 비로그인이면 로그인 유도를 표시한다.

페이지네이션

기존 beans.html의 페이지 버튼 스타일을 재사용한다. 페이지 이동 시 노트 섹션 상단으로 스크롤하되 페이지 전체를 위로 올리지 않는다. URL은 변경하지 않는다 — 로트 상세의 주 상태는 로트 id이며, 노트 페이지 번호까지 주소에 넣으면 공유 링크의 의미가 흐려진다.

화면 — notes.html

노트 작성 폼에 공개 여부 토글을 추가한다. 기본값은 공개.

토글 옆에 한 줄 안내: "공개하면 다른 사용자가 이 로트 페이지에서 내용을 볼 수 있습니다. 비공개로 두면 아이디와 별점만 표시됩니다."

내 노트 목록의 각 항목에도 공개·비공개 상태를 배지로 표시하고, 수정 모달에서 변경할 수 있게 한다.

개인정보 관점

작성자 식별자로 아이디를 그대로 노출한다. 별도 닉네임 필드가 없으므로 불가피하나, 로그인 아이디가 공개된다는 점은 보안상 트레이드오프다. 회원가입 화면에 이 사실을 안내하거나, 여유가 있다면 users에 display_name을 추가해 분리하는 편이 낫다. 이 판단 과정을 보고서에 기록한다.

하지 말 것
비공개 노트의 세부 내용을 응답에 포함시킨 뒤 프론트에서 숨기기
전체 노트를 받아 클라이언트에서 페이지네이션
노트 목록을 로그인 필수로 만들기 (비로그인도 열람 가능해야 한다)
평균 별점을 현재 페이지 기준으로 계산하기
보고서 소재
응답 단계에서의 데이터 최소화 — 화면에서 가리는 것과 서버에서 내려보내지 않는 것의 차이
동일 리소스의 조건부 노출 — 본인 여부에 따라 같은 엔드포인트가 다른 필드를 반환하는 설계
인증 선택적 엔드포인트 — 로그인하지 않아도 동작하되, 로그인 시 응답이 달라지는 API
인덱스 추가 근거 — (bean_id, created_at DESC) 복합 인덱스와 주 질의 패턴
기존 데이터 마이그레이션 정책 — 기본값을 공개로 둘 것인가 비공개로 둘 것인가
콘텐츠