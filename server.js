...
if (!process.env.SESSION_SECRET) {
  throw new Error('SESSION_SECRET이 없다. .env.example을 복사해 .env를 만들고 값을 채운다.');
}
...
app.use(session({
  ...
  resave: false,              // 변경 없으면 다시 저장하지 않는다
...
// public/ 아래 HTML·CSS·JS·JSON을 그대로 내보낸다.
app.use(express.static(path.join(__dirname, 'public')));

// --- API 라우터 마운트 ---------------------------------------------
// 각 라우터를 만들면 import하고 아래 주석을 푼다.
...
// 에러 핸들러는 인자 4개짜리여야 Express가 알아본다.
